...
def run_plot_D(Network_new,DIAM_NEW):
    # Correr red y extraer dato para la Grafic
    dic_nodos, conexiones , coord_x, coord_y, P, cota = extraer_nodos(Network_new )
    ...
    try:
        ...
        if cantidad_tubos > 0:
            ...
            with c1:
                ...
                with st.container():
                    st.subheader('Velocidad de diseño(m/s)')
                    velocidad = st.slider(" velocidad de diseño (m/s)", min_value=0.5, max_value=4.0, step=0.5)
                    st.subheader('Presión mínima (mca)')
                    pmin = st.slider(" Presión mínima (mca)", min_value=1, max_value=50, step=5)


            # 🧮 Calcular nuevos diámetros
            DIAM_NEW = calcular_diametros(Network_orig, velocidad)

            # 🔧 Reemplazar diámetros en la red
            Network_new = replace_diam(Network_orig, DIAM_NEW)
            report      = Network_new.run()
            ...
            with c2:
                ...
                with col1:
                    run_plot_presiones(Network_new,pmin)
                    dic_nodos, conexiones , coord_x, coord_y, P, cota = extraer_nodos(Network_new)
                    run_tabla_resultados ([node.id for node in on.get_nodes(Network_new)], P, 'Nodo', 'P (mca)')

                    run_plot_D(Network_new,DIAM_NEW)
                    run_tabla_resultados ([link.id for link in on.get_links(Network_new)], DIAM_NEW, 'Tuberia', 'D(mmm)')
                    run_tabla_resultados ([link.id for link in on.get_links(Network_new)], aproximar_diametros_comerciales(DIAM_NEW),  'Tuberia', 'D(pulg)')
                ...
                with col2:
                    run_plot_presiones_psi(Network_new,pmin)
                    run_tabla_resultados([node.id for node in on.get_nodes(Network_new)], [round(p * 0.0142233, 2) for p in P], 'Nodo', 'P (psi)')

                    run_plot_Q(Network_new)
                    run_tabla_resultados ([link.id for link in on.get_links(Network_new)],[abs(round(report.flow[i.id].mean(), 2))  for i in on.get_links(Network_new)],  'Tuberia', 'Q(lps)')
